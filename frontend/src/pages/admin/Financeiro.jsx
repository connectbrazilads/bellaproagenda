...
const FINANCEIRO_MOBILE_MODULES = [
  ...
  { id: 'stat-lucro', label: 'Lucro liquido' },
  { id: 'chart-faturamento', label: 'Evolucao do faturamento' },
  { id: 'mix-receita', label: 'Mix de receita' },
  { id: 'relatorio-caixa', label: 'Relatorio diario de caixa' },
  { id: 'caixa-operacional', label: 'Caixa operacional' },
  { id: 'historico-turnos', label: 'Historico de turnos' },
  { id: 'fechamento-diario', label: 'Fechamento diario' },
...
export default function Financeiro() {
  ...
  function renderMobileModule(moduleId) {
    switch (moduleId) {
      ...
      case 'stat-comissoes':
        return <StatCard label="Comissoes" value={data?.totalComissoes} icon={<Users />} tone="gold" />;
      case 'stat-despesas':
        return <StatCard label="Despesas" value={data?.totalDespesas} icon={<TrendingDown />} tone="ink" />;
      case 'stat-lucro':
        return <StatCard label="Lucro liquido" value={data?.lucroLiquido} icon={<Wallet />} tone="green" />;
      case 'chart-faturamento':
        return (
          <div className="rounded-[32px] border border-white/8 bg-[rgba(41,31,37,0.94)] p-6 shadow-[0_24px_60px_rgba(0,0,0,0.24)]">
            ...
            <div className="h-[340px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data?.chartData || []}>
                  ...
                  />
                  <Area type="monotone" dataKey="value" stroke="#e99ba8" strokeWidth={3} fill="url(#gradRoseMobile)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      case 'desempenho-equipe':
        return (
          <section ref={equipeMobileRef} className="overflow-hidden rounded-[32px] border border-white/8 bg-[rgba(41,31,37,0.94)] shadow-[0_24px_60px_rgba(0,0,0,0.24)]">
            <div className="flex flex-col gap-4 border-b border-white/8 p-6">
              <div className="flex items-center gap-3">
    ...
    <div ref={pageRef} className="mx-auto flex max-w-7xl flex-col gap-6 pb-16 lg:gap-8">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-[rgba(9,6,10,0.72)] px-3 py-6">
          <div className="max-h-[88vh] w-full max-w-xl overflow-hidden rounded-[32px] border border-white/10 bg-[rgba(28,23,31,0.98)] shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
            <div className="flex items-start justify-between gap-4 border-b border-white/8 p-6">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-[#e99ba8]">Modo mobile</p>
                <h2 className="mt-2 font-['Playfair_Display'] text-3xl text-[#faf7f6]">Personalizar painel</h2>
                <p className="mt-3 text-sm leading-6 text-[#c7adb4]">
                  Escolha a ordem dos modulos e esconda o que nao fizer sentido para voce.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setShowMobileLayoutEditor(false)}
                className="rounded-full border border-white/8 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#c7adb4]"
              >
                Fechar
              </button>
            </div>
            ...
            <div className="max-h-[60vh] space-y-3 overflow-y-auto p-6">
              {mobileLayout.map((item, index) => (
                <div
                  key={item.id}
                  className={cn(
                    'flex items-center gap-3 rounded-[24px] border p-4',
                    item.visible
                      ? 'border-white/8 bg-[rgba(255,255,255,0.03)]'
                      : 'border-white/5 bg-[rgba(255,255,255,0.02)] opacity-70'
                  )}
                >
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      item.visible
                        ? 'border-[rgba(233,155,168,0.22)] bg-[rgba(233,155,168,0.12)] text-[#f7c1b6]'
                        : 'border-white/8 text-[#8f7880]'
                    )}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-[#faf7f6]">{item.label}</p>
                    <p className="mt-1 text-[11px] uppercase tracking-[0.18em] text-[#9f848d]">
                      {item.visible ? 'Visivel no painel' : 'Oculto no painel'}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={index === 0}
                      className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-white/8 text-[#c7adb4] disabled:opacity-35"
                    >
            ...
            <div className="flex flex-wrap justify-between gap-3 border-t border-white/8 p-6">
              <button
                type="button"
                onClick={resetMobileLayout}
                className="inline-flex min-h-[48px] items-center justify-center rounded-full border border-white/8 px-5 text-sm font-semibold uppercase tracking-[0.18em] text-[#c7adb4]"
              >
                Restaurar padrao
              </button>
              <button
                type="button"
                onClick={() => setShowMobileLayoutEditor(false)}
                className="inline-flex min-h-[48px] items-center justify-center rounded-full bg-[linear-gradient(135deg,#e99ba8_0%,#f7c1b6_100%)] px-6 text-sm font-semibold uppercase tracking-[0.18em] text-[#20191f]"
              >
                Aplicar
              </button>
            </div>
      ...
      <div className="space-y-4">
        {visibleMobileModules.length ? (
          ...
          <div className="rounded-[28px] border border-dashed border-white/10 bg-[rgba(41,31,37,0.7)] p-6 text-center">
            <p className="text-sm font-semibold text-[#faf7f6]">Nenhum modulo visivel no mobile.</p>
            <p className="mt-2 text-sm text-[#9f848d]">Abra a personalizacao e ative os blocos que voce quer acompanhar.</p>
            <button
              type="button"
              onClick={() => setShowMobileLayoutEditor(true)}
              className="mt-5 inline-flex min-h-[48px] items-center justify-center rounded-full bg-[rgba(233,155,168,0.14)] px-5 text-sm font-semibold uppercase tracking-[0.18em] text-[#f7c1b6]"
            >
              Ajustar painel
            </button>
          </div>
      ...
      <section ref={equipeDesktopRef} className="rounded-[32px] border border-white/8 bg-[rgba(41,31,37,0.94)] shadow-[0_24px_60px_rgba(0,0,0,0.24)] overflow-hidden">
