...
|---|---|---:|---:|---|---|
| Jobs agendados em processo | Runtime | Alto | Média | Lembretes dependem do processo da API | Extrair worker dedicado e evitar cron duplicado em múltiplas instâncias |
...
| JWT em `localStorage` | Segurança | Alto | Média | Sessão persistente no navegador | Evoluir para estratégia com `httpOnly` cookie ou endurecer camada web e CSP |
...
| Rate limiting insuficiente | Segurança | Médio | Média | Não há proteção forte documentada para login e booking público | Adicionar rate limiting por IP, tenant e tipo de endpoint |
